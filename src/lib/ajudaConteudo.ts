...
export const ARTIGOS_AJUDA: ArtigoAjuda[] = [
  ...
  },
  {
    ...
    regras: [
      ...
      "Quando o selo de um grupo é \"Parcial\" (mistura de lançamentos pagos e não pagos naquele mês), aparece também, entre o nome do grupo e o selo, a soma em vermelho do que ainda falta pagar daquele grupo no mês.",
  ...
  },
  {
    ...
    regras: [
      ...
      "Um lançamento criado a partir de um recebido em Lançamentos Compartilhados mostra, ao passar o mouse sobre a coluna Reembolso, o credor real e o valor real do lançamento original na conta de quem enviou (o Credor e o Valor exibidos aqui já vêm ajustados: nome de quem enviou e valor já rateado pelo modo do reembolso).",
  ...
  },
  {
    ...
    regras: [
      ...
      "Acima da tabela, tanto em Envios quanto em Recebidos, aparece a quantidade de lançamentos e a soma dos valores do reembolso (mesmo padrão usado em Contas a Pagar → Lançamentos): em Envios soma o Valor do Reembolso de todos os itens da lista atual (Pendentes ou Já Enviados, conforme o filtro Status); em Recebidos soma o Valor de todos os itens ainda \"Não Lançado\" do mês.",
      ...
      "Em Recebidos só aparecem os lançamentos ainda \"Não Lançado\" (o sistema não guarda histórico do que já foi lançado) — a lista também traz as colunas Credor, Observação, Grupo, Aplicação, Parcela (ex.: \"1/1\", \"1/3\" ou \"Fixa\") e Reembolso, com as mesmas informações do lançamento original do outro usuário. O cabeçalho tem o mesmo checkbox com setinha (\"Todas as páginas\") usado em Envios, e o mesmo botão \"Excluir\" para excluir vários recebidos de uma vez. Para conta fixa, a coluna Data mostra o vencimento daquela ocorrência do mês, já que conta fixa não tem \"data da compra\".",
      "Em Recebidos, passar o mouse sobre a coluna Reembolso mostra o credor real e o valor real do lançamento original na conta de quem enviou — útil porque o Credor exibido na lista é o nome de quem enviou (não o credor real da compra) e o Valor já vem rateado pelo modo do reembolso.",
      ...
      "Grupo e Aplicação são escolhidos direto nas colunas da tabela (um seletor em cada célula), sem precisar abrir nada: a célula fica com fundo vermelho enquanto vazia e vira verde assim que um valor é escolhido. Grupo sempre começa vazio (não existe sugestão de grupo de quem enviou); Aplicação já vem pré-preenchida (e verde) quando a sugestão de quem enviou já existe na sua lista, ou vazia (vermelha) quando não existe.",
      "Um lançamento de origem que virou várias parcelas (várias linhas na tabela, ex. \"1/3\", \"2/3\", \"3/3\") tem uma única escolha de Grupo/Aplicação compartilhada entre todas elas — mudar numa linha atualiza as demais linhas do mesmo lançamento automaticamente.",
      "O botão Lançar fica desativado enquanto qualquer um dos lançamentos marcados ainda tiver Grupo ou Aplicação vazio (vermelho); passar o mouse sobre o botão desativado explica o motivo. Assim que todos os marcados estiverem com as duas colunas verdes, o botão libera e um único clique lança todos de uma vez, cada um no Grupo e na Aplicação escolhidos na própria linha.",
      "Quando a aplicação sugerida por quem enviou ainda não existe na sua lista, aparece um link \"Criar '...' (sugestão)\" abaixo do seletor de Aplicação daquela linha; ao clicar, a aplicação é criada na sua lista de Configurações (com esse nome exato) e já fica selecionada — não é preciso ir até Configurações separadamente.",
  ...
  },
  {
    ...
    resumo:
      "Relatório com período customizável (De/Até) e busca por nome de credor, com totais por grupo, por aplicação e por reembolso, além da lista completa de lançamentos (na impressão/PDF). Recurso do plano Premium.",
    regras: [
      "Por padrão, o período já abre preenchido com o mês atual inteiro.",
      "O campo de busca por credor soma vários nomes ao mesmo tempo: digite o nome exato de um credor (ex.: \"Lisley\") e tecle Enter, ou clique numa das sugestões da lista, para transformá-lo numa etiqueta abaixo do campo — o relatório passa a somar os lançamentos de todos os credores nas etiquetas, não só do último. Digite outro nome (ex.: \"Lisley Soares\") e repita: agora os lançamentos de \"Lisley\" e de \"Lisley Soares\" são somados juntos, cada um mantendo seus próprios lançamentos. Cada etiqueta tem um \"×\" para remover, e a lista atualiza o total e os resumos na hora. A comparação é pelo nome exato do credor, não por parte do nome — buscar \"Ana\" não traz \"Ana Paula\" junto; para somar as duas, adicione as duas como etiquetas separadas.",
      "O reembolso é calculado ao vivo, a partir das contas do período e da configuração de cada pessoa — por isso é sempre a referência \"correta\" para conferir os valores, mesmo que a tela de Recebimentos ainda não tenha os lançamentos gerados.",
    ...
    imagemAlt: "Relatório Modelo I com período customizável e busca por credor",
    marcadores: [
      { numero: 1, xPct: 8.7, yPct: 24.7, texto: "Busca por nome de credor — Enter ou clique numa sugestão soma vários credores de uma vez." },
  ...
  },
  {
    ...
    resumo:
      "Relatório de um mês fixo (sem período customizável), sem o filtro de aplicação, mostrando só os lançamentos ainda não pagos do mês selecionado. Recurso do plano Premium.",
    regras: [
      ...
      "Não existe filtro de Aplicação aqui; é possível filtrar só por Grupo e Reembolso.",
      "Os resumos (por grupo, por aplicação e por reembolso) e a lista completa de lançamentos na impressão/PDF seguem a mesma lógica dos demais modelos, mas sempre restritos ao que ainda está pendente no mês.",
      "\"Compartilhar\" gera um PDF e tenta abrir o menu nativo de compartilhamento do dispositivo (WhatsApp, e-mail, etc.); se o navegador não suportar, o PDF é baixado diretamente.",
    ],
    validacoes: [],
    imagem: "/ajuda/relatorio-modelo-ii.png",
    imagemAlt: "Relatório Modelo II com aviso de lançamentos não pagos do mês",
    marcadores: [
  ...
  },
  {
    ...
    resumo:
      "Relatório do mês selecionado com um Top 10 dos lançamentos mais caros do mês logo abaixo dos totais. Recurso do plano Premium.",
    regras: [
      "Os filtros (Grupo, Aplicação, Reembolso), os totais por grupo/aplicação/reembolso e a lista completa de lançamentos na impressão/PDF seguem a mesma lógica dos demais modelos — por padrão todos os itens de cada filtro vêm selecionados.",
      ...
      "O Top 10 nunca mostra lançamentos do grupo \"Provisões\" nem de grupos chamados Fixas, Fixa, Mensal ou Mensais (case-insensitive) — contas recorrentes e provisões não são o tipo de gasto avulso que esse ranking existe para destacar. Esses lançamentos continuam normalmente nos totais e na lista completa, só ficam fora do Top 10.",
      ...
      "\"Compartilhar\" gera um PDF e tenta abrir o menu nativo de compartilhamento do dispositivo (WhatsApp, e-mail, etc.); se o navegador não suportar, o PDF é baixado diretamente.",
      "Os submenus (Modelo I a III) e a barra de filtros/ações ficam fixos no topo da tela enquanto os quadrados de totais passam por baixo deles rolando a página; assim que os quadrados terminam de passar, a barra solta e rola normalmente junto com o Top 10.",
    ],
    validacoes: [],
    imagem: "/ajuda/relatorio-modelo-iii.png",
    imagemAlt: "Relatório Modelo III com Top 10 dos lançamentos mais caros do mês",
    marcadores: [
      { numero: 1, xPct: 19.1, yPct: 20, texto: "Filtros por grupo, aplicação e reembolso." },
  ...
  },
  {
    ...
    titulo: "Dashboard",
    resumo:
      "Visão gráfica com filtro de Tipo (Contas a pagar/Contas a receber) e quatro gráficos (aplicação, grupo, tipo de reembolso e lançamentos parcelados) em pizza ou barras. Recurso do plano Premium.",
    regras: [
      "O filtro \"Tipo\", junto aos demais filtros, alterna entre Contas a Pagar (marcado por padrão) e Contas a Receber — a escolha define quais outros filtros aparecem e os dados exibidos nos quatro quadros.",
      "O primeiro quadro (\"Por aplicação\"/\"Por observação\") mostra, abaixo do título, a quantidade de lançamentos (ou recebimentos) e o total pago (ou recebido) no mês filtrado.",
      ...
      "Clicar em um dos quatro quadros recolhe os quatro (mantendo o tamanho padrão deles) para uma coluna estreita à esquerda e abre, à direita, uma lista detalhada dos lançamentos daquele quadro, agrupados pela mesma categoria do gráfico. Cada título do grupo mostra o subtotal e começa recolhido — clique no título para exibir (ou ocultar de novo) os lançamentos daquele grupo, com data da compra, credor/origem, observação e valor de cada um. A lista rola dentro do próprio painel quando não cabe tudo, sem alterar o tamanho dos quadros.",
      "No canto do painel de detalhe, o botão de maximizar expande a lista para ocupar todo o espaço (ocultando os quadros por enquanto); o botão ao lado fecha o painel e volta à grade normal. Clicar novamente no quadro selecionado ou em outro quadro também fecha o painel.",
    ],
    validacoes: [],
    imagem: "/ajuda/dashboard.png",
    imagemAlt: "Dashboard analítico com gráficos de aplicação, grupo, reembolso e parcelamento",
    marcadores: [
      { numero: 1, xPct: 15.9, yPct: 12.4, texto: "Filtro de Tipo (Contas a pagar/Contas a receber) e demais filtros." },
      { numero: 2, xPct: 86.1, yPct: 30.2, texto: "Alterna o tipo de gráfico entre pizza (padrão) e barras." },
      { numero: 3, xPct: 11.7, yPct: 36.1, texto: "Distribuição por aplicação, com quantidade de lançamentos e total pago no mês." },
      { numero: 4, xPct: 51.2, yPct: 36.1, texto: "Distribuição por grupo." },
      { numero: 5, xPct: 11.7, yPct: 62.4, texto: "Distribuição por tipo de reembolso." },
      { numero: 6, xPct: 51.2, yPct: 62.4, texto: "Quanto do total do mês é parcelado e quanto é à vista." },
    ],
  },
  {
    ...
    regras: [
      ...
      "Um grupo pode ser marcado como favorito (ícone de estrela) — ele já vem selecionado ao abrir o formulário de novo lançamento (no site e no app mobile), mas qualquer outro grupo continua podendo ser escolhido na hora. No site, depois de escolher outro grupo, esse grupo passa a vir selecionado nos próximos lançamentos (mesmo trocando de aba) até você sair da tela de Contas a Pagar — ao voltar, vale de novo o favorito. Só um grupo pode ser favorito por vez: marcar outro troca automaticamente o anterior. Remover o grupo favorito também remove o favorito.",
      "Cada grupo pode ter uma cor (bolinha ao lado da estrela): roxo, vermelho, amarelo, laranja, verde ou azul. A cor aparece só no campo Grupo da tela de Novo lançamento (no grupo selecionado e na lista de opções); sem cor escolhida, o grupo continua no estilo padrão.",
    ...
    marcadores: [
      ...
      { numero: 5, xPct: 88.4, yPct: 29.5, texto: "Interruptor de modo escuro (Premium)." },
      {
        numero: 6,
        xPct: 25.6,
        yPct: 53.7,
        texto: "Grupos — cadeado protege o item, balão edita a observação, estrela marca o favorito.",
      },
